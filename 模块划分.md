...
| F6 单元格复制      | ✅ 已完成 | `Core/Copy/IClipboard.cs`（新建，剪贴板写入端口）、`Core/Copy/CellCopyService.cs`（新建，`CopyStatus`/`CopyResult`/`Copy` 纯逻辑）、`App/Services/WinClipboard.cs`（新建，WinForms 实现）、`Views/DashboardView.cs`（`CellClick` 接线）；对应 `CellCopyServiceTests`                                                                                                                                                                              |
| F7 CSV 导出     | ✅ 已完成 | `Core/Export/CsvExporter.cs`（新建，`Write`/`BuildDefaultFileName` 纯逻辑：UTF-8 BOM、`TableColumns` 派生表头/数据、过滤全空列、全空保列、RFC 4180 转义、CSV 公式注入中和）、`Views/DashboardView.cs`（导出按钮：`SaveFileDialog`→临时文件原子写→`Task.Run` 后台写盘）、`Core.Tests/CsvExporterTests`+`CsvExporterEdgeCaseTests`+`CsvExporterIntegrationTests`（BOM/派生/过滤/转义/公式中和/并发/端到端链路）                                                                                                                                      |
| F8 会话管理       | ✅ 已完成 | `Core/Session/LogoutDecider.cs`（新建，`LogoutOutcome` + `FromException`/`FromSettingsSaved`/`FromManualLogout` 纯逻辑与文案单一数据源）、`Views/DashboardView.cs`（`PerformLogout`：取消 CTS→`ClearToken`→弹窗→`ShowLogin`；左栏「设置」「退出登录」按钮接线；401 分支跳登录页）、`MainForm.cs` 注入外壳与存储；对应 `LogoutDeciderTests`（9 用例）+ `FetchIntegrationTests` 会话失效端到端（2 用例） |
| F9 版本号展示      | ✅ 已完成 | `AppConfig.Version` + `MainForm` 标题                                                                                                                                                |

**当前可验证项**：`\scripts\test.ps1`（全量用例全绿；测试项目含 `tests/Core.Tests` 与 `tests/App.Tests`，F5 新增 `TableGridTests`，F6 新增 `CellCopyServiceTests`）；`\scripts\dev.ps1` 可进入登录页，用真账号 `000/0000` 登录后进入主面板，切换仓库/对比项并点「刷新数据」可拉取库存差异，**12 列数据表格随即渲染**（F5 已交付），**点击任意带值单元格即复制到剪贴板并在状态栏提示**（F6 已交付），**点左栏「退出登录」回登录页、改地址保存后自动回登录页、token 失效（401）自动跳登录页**（F8 已交付）。
...
> **完成说明（2026-09-22）**：原计划的「在 `DashboardView.CellClick` 内直接调 `Clipboard.SetText`」改为**端口 + 服务**两层，与 F5 `TableGrid` 的「纯逻辑下沉 Core、零 UI 依赖可脱离界面单测」做法保持一致：
>
> - `src/Core/Copy/IClipboard.cs`（新建）：剪贴板写入端口，Core 层不依赖 UI。
> - `src/Core/Copy/CellCopyService.cs`（新建）：`CopyStatus`（`Copied` / `Empty` / `Failed`）、`CopyResult`（状态 + 文本 + 提示文案）与 `CellCopyService.Copy`。空值不写剪贴板；写入抛异常统一收口为 `Failed` 不向外抛；构造入参为 `null` 抛 `ArgumentNullException`。
> - `src/App/Services/WinClipboard.cs`（新建）：`IClipboard` 的 WinForms 实现。
> - `src/App/Views/DashboardView.cs`（修改）：构造时注入 `CellCopyService`，`CellClick` 接线；表头点击（行列索引为负）忽略；成功置绿字「已复制: xxx」，失败置红字并 `Trace.WriteLine` 记日志。
> - `tests/Core.Tests/CellCopyServiceTests.cs`（新建）：正常复制 / 原样保真 / 空值与纯空白 / 超长文本 / 特殊字符与 Unicode / 剪贴板占用异常收口 / 构造入参为空 / 并发调用。

| 任务   | 文件                                        | 内容要点                                                    |
| ---- | ----------------------------------------- | ------------------------------------------------------- |
| F6-1 | `src/Core/Copy/IClipboard.cs`（新建）         | 剪贴板写入端口，隔离 UI 依赖                                        |
| F6-2 | `src/Core/Copy/CellCopyService.cs`（新建）    | `Copy`：空值 → `Empty`；写入成功 → `Copied`；异常 → `Failed`（不抛出）    |
| F6-3 | `src/App/Services/WinClipboard.cs`（新建）     | 端口的 WinForms 实现（`Clipboard.SetText`）                     |
| F6-4 | `src/APP/Views/DashboardView.cs`（修改）       | `CellClick` 接线：空值不动作、成功/失败分别置状态栏、失败记日志                    |
| F6-5 | `tests/Core.Tests/CellCopyServiceTests.cs`（新建） | 正常 / 边界（空值、纯空白、超长、特殊字符）/ 异常（剪贴板占用、入参为空）/ 并发             |

**验收**：点带值单元格→状态栏「已复制: xxx」且粘贴内容一致（含负数减号）；点空单元格无变化；点表头不报错；剪贴板被占用时红字提示且程序不崩溃。`test.ps1` 中 `CellCopyServiceTests` 全绿。

**范围边界**：CSV 导出 F7；左栏「设置」「退出登录」与 401 自动跳登录 F8。
...
> **完成说明（2026-09-23）**：退出决策下沉为 Core 纯逻辑 `src/Core/Session/LogoutDecider.cs`（零 UI 依赖、可脱离界面单测），与 F6 `CellCopyService`、F5 `TableGrid` 的做法一致；`DashboardView` 左栏新增「设置」「退出登录」按钮，`SettingsForm` 保存成功与手动退出均走统一的 `PerformLogout`，`OnRefreshClick` 的 `UnauthorizedException` 分支由「仅提示」改为跳回登录页；`MainForm.ShowDashboard` 注入外壳与 `IBaseUrlStore`。新增 `LogoutDeciderTests`（9 用例）与 `FetchIntegrationTests` 会话失效端到端（2 用例）。
>
> **范围边界**：`SettingsForm` 自身逻辑归 F2；`ClearTokenIf` 防旧 401 抹新 token 归 F1/F4。
