...
**当前可验证项**：`\scripts\test.ps1`（全量 199 用例全绿，F4 新增 `ApiClientTests` 拉数据用例 + `FetchIntegrationTests` 2 个用例）；`\scripts\dev.ps1` 可进入登录页，用真账号 `000/0000` 登录后进入主面板，切换仓库/对比项并点「刷新数据」可拉取库存差异（表格渲染由 F5 提供）。
