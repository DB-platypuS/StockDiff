...
**当前可验证项**：`\scripts\test.ps1`（全量用例全绿；测试项目含 `tests/Core.Tests` 与 `tests/App.Tests`，F5 新增 `TableGridTests`，F6 新增 `CellCopyServiceTests`）；`\scripts\dev.ps1` 可进入登录页，用真账号 `000/0000` 登录后进入主面板，切换仓库/对比项并点「刷新数据」可拉取库存差异，**12 列数据表格随即渲染**（F5 已交付），**点击任意带值单元格即复制到剪贴板并在状态栏提示**（F6 已交付）。
