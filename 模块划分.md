...
| **F2 接口地址设置与持久化** | 改接口地址、重启后仍生效         | `Dialogs/SettingsForm.cs`、`Settings/AppSettings.cs`                                                                                               | F1（需从登录页进入）      |
...
| F1 登录认证      | ☐ 未开始  | 缺 `Settings/AppSettings.cs`、`Api/ApiClient.cs`、`Views/LoginView.cs`；`MainForm.cs` 目前仅窗体外壳，无视图切换                                 |
| F2 接口地址设置与持久化 | ☐ 未开始  | 缺 `Dialogs/SettingsForm.cs`                                                                                               |
...
**当前可验证项**：`\scripts\test.ps1`（Core 层 5 个测试类）；`\scripts\dev.ps1` 可启动空白窗体。
**待办提醒**：`AppSettings` 落盘实现时须采用「写临时文件再 `File.Move(overwrite: true)`」的原子写，避免进程被强杀时损坏 `settings.json`（参考实现思路见 `设计文档.md` 对应章节）。
